...
void plagiarism_checker_t::add_submission(std::shared_ptr<submission_t> __submission){
    ...
    for (auto other_submission: this->submissions){
        if (other_submission.second < curr_time - std::chrono::seconds(1)){
            ...
            std::thread t(&plagiarism_checker_t::check_plagiarism,__submission, other_submission.first);
...
// First file is to be reported as plag by default
...
// Join all threads when the object is destroyed
